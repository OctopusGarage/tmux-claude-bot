  ...
  useInput(
    (ch, key) => {
      ...
      if (mode === "autopilot") {
        ...
        const actions = autopilotActionList();
        if (key.return) {
          const a = actions[apSel];
          if (a && selected) void client.autopilot(selected.session, a.verb);
          return;
        }
        if (key.upArrow || ch === "k") setApSel((i) => Math.max(0, i - 1));
        else if (key.downArrow || ch === "j") setApSel((i) => Math.min(actions.length - 1, i + 1));
        return;
      }
      // list mode
      if (ch === "q") {
              ...
              : mode === "logs" || mode === "sysload"
                ...
                : mode === "autopilot"
                  ? pane(
                      ...
                      <Box flexDirection="column">
                        <Text color="gray">
                          Delegates the selected session to the Loop Supervisor.
                        </Text>
                        <Box flexDirection="column">
                          {autopilotActionList().map((a, i) => (

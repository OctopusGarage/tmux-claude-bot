...
import { writeTerminalModeReset } from "../shared/utils/terminal-modes.js";
 ...
 * blank screen. The `a` key suspends the TUI, drops into the real session pane, and
...
export async function runTui(): Promise<void> {
  ...
  const resetTerminal = (): void => writeTerminalModeReset(process.stdout);
  ...
  try {
    for (;;) {
      ...
      await waitUntilExit();
      resetTerminal();
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- onAttach mutates this from a React event callback before waitUntilExit resolves.
      if (!attachTo) break;

...
/**
 * Messages for the interactive setup wizards (`pnpm setup`, `pnpm setup:lark`).
 * Kept separate from the chat-UI catalog (Messages) because the
...
const en: SetupMessages = {
  ...
  envExists: (path) =>
    `${path} already exists. Re-run with --reconfigure (pnpm setup:reconfigure) to edit it.`,
  yesNeedsToken:
  ...
  allowedIdsEmpty:
    "TELEGRAM_ALLOWED_USER_IDS is empty — the bot will reject ALL messages until you set it (pnpm setup:reconfigure).",
  ...
  botAlreadyRunningHint:
    "If the bot is already running, stop it first (pnpm service:uninstall) or enter your id manually.",
  ...
  larkRestartHint: (cmd) => `Restart the bot to apply: ${cmd} (in dev, restart pnpm dev)`,
  ...
  voiceSkipHint:
    "Press ENTER to skip for now. To enable later: `pnpm whisper:install` auto-fills this. (No need to type a path by hand.)",
...
const zh: SetupMessages = {
  ...
  envExists: (path) =>
    `${path} 已存在。重新运行并加 --reconfigure（pnpm setup:reconfigure）来修改。`,
  ...
  allowedIdsEmpty:
    "TELEGRAM_ALLOWED_USER_IDS 为空 —— 在你设置它之前，bot 会拒绝所有消息（pnpm setup:reconfigure）。",
  ...
  botAlreadyRunningHint:
    "如果 bot 已在运行，请先停止它（pnpm service:uninstall），或手动输入你的 id。",
  ...
  larkRestartHint: (cmd) => `重启 bot 生效：${cmd}（开发态则重启 pnpm dev）`,
  ...
  voiceSkipHint:
    "按回车先跳过。稍后启用：运行 `pnpm whisper:install` 会自动填好此项。（无需手动输入路径。）",
...
const yue: SetupMessages = {
  ...
  envExists: (path) =>
    `${path} 已經存在。重新執行並加 --reconfigure（pnpm setup:reconfigure）嚟修改。`,
  ...
  allowedIdsEmpty:
    "TELEGRAM_ALLOWED_USER_IDS 係空 —— 喺你設定佢之前，bot 會拒絕所有訊息（pnpm setup:reconfigure）。",
  ...
  botAlreadyRunningHint:
    "如果 bot 已經喺度運行，請先停止佢（pnpm service:uninstall），或者手動輸入你嘅 id。",
  ...
  larkRestartHint: (cmd) => `重啟 bot 生效：${cmd}（開發態則重啟 pnpm dev）`,
  ...
  voiceSkipHint:
    "撳 Enter 暫時略過。之後啟用：執行 `pnpm whisper:install` 會自動填好呢項。（唔使手動輸入路徑。）",
...
const zhTW: SetupMessages = {
  ...
  envExists: (path) =>
    `${path} 已經存在。重新執行並加 --reconfigure（pnpm setup:reconfigure）來修改。`,
  ...
  allowedIdsEmpty:
    "TELEGRAM_ALLOWED_USER_IDS 為空 —— 在你設定它之前，bot 會拒絕所有訊息（pnpm setup:reconfigure）。",
  ...
  captureListenerFailed: (err) => `無法啟動擷取監聽器（${err}）。`,
  botAlreadyRunningHint:
    "如果 bot 已在執行，請先停止它（pnpm service:uninstall），或手動輸入你的 id。",
  noMessageReceived: "未收到訊息。",
  ...
  voiceSkipHint:
    "按 Enter 先略過。稍後啟用：執行 `pnpm whisper:install` 會自動填好此項。（無需手動輸入路徑。）",
  mlxPathPrompt: "mlx_whisper 執行檔路徑（Enter 略過）",
...
const ja: SetupMessages = {
  ...
  envExists: (path) =>
    `${path} は既に存在します。--reconfigure を付けて再実行（pnpm setup:reconfigure）して編集してください。`,
  yesNeedsToken:
  ...
  allowedIdsEmpty:
    "TELEGRAM_ALLOWED_USER_IDS が空です —— 設定するまで bot は全メッセージを拒否します（pnpm setup:reconfigure）。",
  ...
  botAlreadyRunningHint:
    "bot が既に実行中の場合は先に停止（pnpm service:uninstall）するか、id を手動入力してください。",
  ...
  larkRestartHint: (cmd) => `反映するには bot を再起動：${cmd}（開発時は pnpm dev を再起動）`,
  ...
  voiceSkipHint:
    "Enter で今はスキップ。後で有効化：`pnpm whisper:install` がこの項目を自動で埋めます。（パスを手入力する必要はありません。）",
...
const es: SetupMessages = {
  ...
  envExists: (path) =>
    `${path} ya existe. Vuelve a ejecutar con --reconfigure (pnpm setup:reconfigure) para editarlo.`,
  yesNeedsToken:
  ...
  allowedIdsEmpty:
    "TELEGRAM_ALLOWED_USER_IDS está vacío — el bot rechazará TODOS los mensajes hasta que lo definas (pnpm setup:reconfigure).",
  ...
  botAlreadyRunningHint:
    "Si el bot ya está en ejecución, deténlo primero (pnpm service:uninstall) o introduce tu id manualmente.",
  ...
  larkRestartHint: (cmd) => `Reinicia el bot para aplicar: ${cmd} (en dev, reinicia pnpm dev)`,
  ...
  voiceSkipHint:
    "Pulsa Enter para omitir por ahora. Para activarla luego: `pnpm whisper:install` lo rellena automáticamente. (No hace falta escribir una ruta a mano.)",

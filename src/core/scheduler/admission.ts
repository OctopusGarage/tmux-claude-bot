...
/**
 * Which queued tasks to admit right now. Highest-priority first; never
 * exceeds `caps[agent]` counting already-running tasks of that agent; skips a
 * paused pool and any agent with no configured cap. An optional predicate can
 * decline a queued candidate before it reserves projected capacity.
 */
export function tasksToAdmit(
  tasks: TaskState[],
  caps: Partial<Record<AgentKind, number>>,
  pools: Record<string, PoolState>,
  canAdmit?: (task: TaskState) => boolean,
): TaskState[] {
  ...
  for (const t of queued) {
    ...
    if (canAdmit !== undefined && !canAdmit(t)) continue;
    admit.push(t);

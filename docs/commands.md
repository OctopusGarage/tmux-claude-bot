...
| `new_free` | Create a free (parallel) project: `new_free [label]` opens a bare tmux session decoupled from any path, so multiple Claudes can run in the same directory |
...
| `sessions` | List resumable Claude sessions for the current project (tap one to resume `claude --resume`) |
...
| `/newfreegroup <path\|name>` | Private chat (p2p) | Like `/newgroup`, but binds the new group to a fresh free session (`tmux_proj_free_<n>`) so it can sit on a directory that already has a group — multiple parallel Claudes on one workspace. Typed-path counterpart of the 🆓 Parallel group button (which is limited to recent projects). |

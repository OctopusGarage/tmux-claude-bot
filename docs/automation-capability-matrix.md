...
| Resource Guardian | `tcb resource status\|incidents\|mode\|profile`; generic config may set enabled/tick only | Existing `/sysload` shows current Guardian view; no new command or button | Existing `/sysload` card shows current Guardian view; no new command or card action | Existing sysload diagnostic through control client | Describes safe rollout and read-only incident inspection | Not exposed | Protect refuses while disabled. Mode/profile are dedicated local commands; incidents are bounded and output is secret-free and tildeified. |

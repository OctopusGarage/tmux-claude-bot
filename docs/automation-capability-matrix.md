...
| Personal configuration admin | `tcb config list/get/set`; secrets redacted, generic writes allowlisted | Setup/settings commands for supported chat preferences | Setup/settings commands for supported chat preferences | Limited preference toggles | Describes setup and config inspection commands | Not exposed | Secret-bearing credentials stay behind setup/dedicated flows, not generic `config set`. |

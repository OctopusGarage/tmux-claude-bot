...
| Runtime Overview and session status | `tcb dashboard` / `--json` is the full local/admin view; `tcb sessions`, `tcb peek`, and `tcb logs` are drilldowns | `/dashboard` uses the same bounded health-first snapshot; `/peek`, `/history`, and `/logs` drill down | Same Runtime Overview and drilldown command intent through Lark handlers/cards | Header summary plus `o` for the read-only Runtime Overview; live pane and logs drill down | MCP-first `tcb.observer.status`; `tcb dashboard --json` fallback | `tcb.observer.status` returns the canonical typed snapshot; `tcb.observer.projects`, `tcb.observer.sessions`, `tcb.observer.queue`, `tcb.observer.logs_query`, and `tcb.observer.loop_reports_list` are drilldowns | All entry points project one neutral snapshot. Partial reads degrade only their domain; no overview surface mutates state. |

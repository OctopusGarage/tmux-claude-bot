...
import { isClaudeProcess } from "../src/core/agents/claude/claude-process.js";
...
describe("findAgentPid (claude matcher)", () => {
  ...
  it("finds claude as a direct child of the pane pid", () => {
    ...
    expect(findAgentPid(rows, 100, isClaudeProcess)).toBe(200);
  });
  ...
  it("matches when the pane pid itself is claude", () => {
    ...
    expect(findAgentPid(rows, 100, isClaudeProcess)).toBe(100);
  });
  ...
  it("does not match an unrelated 'claude'-containing path", () => {
    ...
    expect(findAgentPid(rows, 100, isClaudeProcess)).toBeNull();
  });
  ...
  it("does not loop forever on a cyclic process graph (visited guard)", () => {
    ...
    expect(findAgentPid(rows, 100, isClaudeProcess)).toBe(400);
  });
});
...
describe("detectAgentKind (inspects the live process)", () => {
  it("returns 'codex' when a codex process runs in the pane tree", async () => {
    const r = createConfigResolver(
      fakeProbe({ snapshot: async () => [{ pid: 200, ppid: 100, command: "/bin/codex --x" }] }),
      OPTS,
    );
    expect(await r.detectAgentKind?.("s")).toBe("codex");
  });

  it("returns 'claude' when a claude process runs in the pane tree", async () => {
    const r = createConfigResolver(
      fakeProbe({ snapshot: async () => [{ pid: 200, ppid: 100, command: "/bin/claude --x" }] }),
      OPTS,
    );
    expect(await r.detectAgentKind?.("s")).toBe("claude");
  });

  it("returns null when neither agent runs in the pane", async () => {
    const r = createConfigResolver(
      fakeProbe({ snapshot: async () => [{ pid: 200, ppid: 100, command: "-zsh" }] }),
      OPTS,
    );
    expect(await r.detectAgentKind?.("s")).toBeNull();
  });

 ...
 *   and createProjectSession only sends the `cd` for the session it actually
 *   created — so a session that already has Claude running never gets a stray
 *   `cd` typed into its prompt.
 *
 * We model a race-safe bridge (with a real interleave window) and assert exactly
 * one `cd` is sent and nothing throws, no matter how many callers race.
 */
describe("session create under concurrency", () => {
  ...
  it("createProjectSession: 12 concurrent calls create once and cd exactly once", async () => {
    ...
    expect(sendKeys).toHaveBeenCalledTimes(1); // only the creator cd'd — no stray cd
  ...
  it("reconcileGroupBinding: concurrent messages on a dead session recreate it once", async () => {
    ...
    try {
      ...
      // No throw, the session is alive, and only one caller actually created it.
